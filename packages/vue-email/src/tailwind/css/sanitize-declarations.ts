import type { CssNode, Declaration, FunctionNode, Raw, Value } from 'css-tree'
import {
...
const BORDER_RADIUS_INFINITY_REGEX = /border-radius\s*:\s*calc\s*\(\s*infinity\s*\*\s*1px\s*\)/i
...
export function sanitizeDeclarations(nodeContainingDeclarations: CssNode) {
  walk(nodeContainingDeclarations, {
    ...
    enter(declaration, item, list) {
      ...
      if (BORDER_RADIUS_INFINITY_REGEX.test(generate(declaration))) {
